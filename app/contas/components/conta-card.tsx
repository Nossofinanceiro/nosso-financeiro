  ...
  return (
    <Card className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 gap-4 hover:border-emerald-500/30 transition-colors group">
      <div className="flex items-center gap-4">
        <div className={`p-3 rounded-xl flex items-center justify-center ${isAtiva ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}>
      ...
      <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
        <div className="text-left sm:text-right">
          ...
          <p className={`text-lg font-medium ${saldoExibir >= 0 ? 'text-white' : 'text-red-400'} ${!isAtiva && 'text-slate-400'}`}>
